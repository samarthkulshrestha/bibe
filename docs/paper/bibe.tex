...
Except where we state a pooled number, each $\pm$ is over five model-init seeds on one corpus realization and one train/test split. The $\pm 0.000$ on a deterministic baseline means ``one draw,'' not ``stable,'' and would move under a different data seed, as the same-object-write row in Table~\ref{tab:ladder} does when we vary it. We report data-seed variance for use-after-free and v2; the others are single-realization, and the pooled numbers are transformer-only, since we did not re-run the LSTM across data seeds. Every $\pm$ is a population standard deviation (dividing by $n$); the sample standard deviation would be about 12\% larger at $n = 5$. We run no significance test. The two inequalities we rely on (LSTM at or above the transformer on v1, and model below the oracle everywhere) are large or true by construction, but the smaller gaps we describe as ties.
...
Bidirectional encoders over log and trace sequences are established; LogBERT~\cite{logbert} and DeepLog~\cite{deeplog} are the standard references, and our bidirectional framing is not new. Attention rollout~\cite{abnar2020} is one way to turn attention into an attribution map; we tried it and fell back to raw last-layer, head-averaged attention. Spectrum-based fault localization --- Tarantula~\cite{tarantula} and the Ochiai coefficient applied to fault localization~\cite{ochiai} --- is a classic non-learned baseline. It is blind on our corpora because it ranks functions by cross-trace coverage, and here every function appears in both passing and failing traces; we include it as a floor, not a competitor. Our labels come from AddressSanitizer~\cite{asan}, and the real bug is issue \#322 in the mjs interpreter~\cite{mjs}.

\begin{thebibliography}{9}
\bibitem{abnar2020} Abnar, S. and Zuidema, W. (2020). Quantifying Attention Flow in Transformers. \emph{ACL 2020}.
\bibitem{ochiai} Abreu, R., Zoeteweij, P., and van Gemund, A. J. C. (2007). On the Accuracy of Spectrum-based Fault Localization. \emph{TAICPART-MUTATION 2007}.
\bibitem{deeplog} Du, M., Li, F., Zheng, G., and Srikumar, V. (2017). DeepLog: Anomaly Detection and Diagnosis from System Logs through Deep Learning. \emph{ACM CCS 2017}.
\bibitem{logbert} Guo, H., Yuan, S., and Wu, X. (2021). LogBERT: Log Anomaly Detection via BERT. \emph{IJCNN 2021}.
\bibitem{tarantula} Jones, J. A. and Harrold, M. J. (2005). Empirical Evaluation of the Tarantula Automatic Fault-Localization Technique. \emph{ASE 2005}.
\bibitem{asan} Serebryany, K., Bruening, D., Potapenko, A., and Vyukov, D. (2012). AddressSanitizer: A Fast Address Sanity Checker. \emph{USENIX ATC 2012}.
\bibitem{mjs} mjs embedded JavaScript engine, Cesanta. Issue \#322 (heap-use-after-free, CWE-416). \url{https://github.com/cesanta/mjs}.
\end{thebibliography}
